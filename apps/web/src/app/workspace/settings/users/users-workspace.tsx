...
type ModalMode =
  "create" | "view" | "edit" | "reset" | "deactivate" | "password";
...
export function UsersWorkspace({
  ...
  async function load(
    ...
    mode: "view" | "edit" | "reset" | "deactivate",
  ) {
    setBusy(true);
    try {
  ...
  async function resetPassword() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(await failure(response));
      const result =
        (await response.json()) as ResetIdentityUserPasswordResponse;
      await refresh();
      setSelected(result.user);
      setOneTimePassword(result.temporaryPassword);
      setModal("password");
  ...
  async function userAction(
    user: IdentityUserSummary,
    action: "activate" | "deactivate" | "revoke-sessions",
  ) {
    setBusy(true);
    try {
      const response = await fetch(
  ...
  return (
    <section className="rq-users-workspace">
      ...
      <RqTableShell
        ...
        description="Cuentas, roles globales, estado y actividad de sesiones."
        title="Usuarios"
      >
        {list.items.length === 0 ? (
          <RqEmptyState
          ...
          <table className="rq-table rq-users-table">
            ...
            <tbody>
              {list.items.map((user) => (
                <tr key={user.id}>
                  <td>
                    <span className="rq-users-table__identity">
                      <strong>{user.displayName}</strong>
                      <span>{user.email}</span>
                      {user.mustChangePassword ? (
                        <small>Cambio de contraseña pendiente</small>
                      ) : null}
                    </span>
                  </td>
                  <td>
                  ...
                  <td>
                    <RqStatusBadge
                      tone={user.status === "ACTIVE" ? "success" : "inactive"}
                    >
                  ...
                  <td>
                    <div className="rq-users-table__actions">
                      ...
                      <RqActionButton
                        ...
                        onClick={() => void userAction(user, "revoke-sessions")}
                        tone="danger"
                      >
                        Revocar sesiones
                      </RqActionButton>
                      {user.status === "ACTIVE" ? (
                        <RqActionButton
                          compact
                          disabled={busy}
                          onClick={() => void load(user, "deactivate")}
                          tone="danger"
                        >
                          Desactivar
                        </RqActionButton>
                        ...
                        <RqActionButton
                          compact
                          disabled={busy}
                          onClick={() => void userAction(user, "activate")}
                          tone="affirmative"
                        >
                          Activar
                        </RqActionButton>
          ...
          <section className="rq-project-modal rq-users-modal">
            <header className="rq-project-modal__header">
              <div>
                ...
                <h2 id="users-modal-title">{modalTitle}</h2>
              </div>
              <button
                aria-label="Cerrar"
                disabled={busy}
            ...
            {modal === "password" ? (
              <div className="rq-users-password">
                <p>
                  <strong>
                    Esta contraseña se mostrará una sola vez. El usuario deberá
                    cambiarla al iniciar sesión.
                  </strong>
                </p>
                <output>{oneTimePassword}</output>
                <div className="rq-project-modal__actions">
                  <RqActionButton onClick={() => close()}>
                    Entendido
                  </RqActionButton>
                </div>
              </div>
            ) : modal === "view" && selected ? (
              <div className="rq-users-detail">
                <dl>
                  <div>
                    <dt>Nombre</dt>
                  ...
                  <div>
                    <dt>Último acceso</dt>
                    <dd>{dateTime(selected.lastLoginAt)}</dd>
                  </div>
                  <div>
              ...
              <div className="rq-users-confirm">
                <p>
                  ¿Deseas desactivar a <strong>{selected.displayName}</strong>?
                  No podrá iniciar sesión y sus sesiones abiertas serán
                  revocadas.
                </p>
                <div className="rq-project-modal__actions">
                  ...
                  <RqActionButton
                    disabled={busy}
                    onClick={() => void userAction(selected, "deactivate")}
                    tone="danger"
                  >
              ...
              >
                ...
                <p>
                  Se revocarán todas las sesiones y se exigirá cambio al
                  iniciar.
                </p>
                <div className="rq-project-modal__actions">
                  <RqActionButton disabled={busy} onClick={() => close()}>
              ...
              <form
                className="rq-project-form rq-users-form"
                onSubmit={(event) => {
                  event.preventDefault();
              ...
              >
                ...
                <fieldset className="rq-users-roles">
                  <legend>Roles globales</legend>
                  {roles.map((role) => (
                    <label key={role.id}>
                      <input
