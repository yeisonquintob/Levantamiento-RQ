...
pkill -TERM -f "nx( |.* )serve operations-service" >/dev/null 2>&1 || true
...
pkill -TERM -f "operations-service:serve:development" >/dev/null 2>&1 || true
...
pkill -TERM -f "apps/operations-service/dist/main.js" >/dev/null 2>&1 || true
pkill -TERM -f "apps/gateway/dist/main.js" >/dev/null 2>&1 || true

for port in 4200 3000 3001 3002 3003 3004 3005 3007 3008; do
...
rm -rf "$PID_DIR"
echo "✓ Identity, Projects, Sources, Documents, AI Analysis, Workflow, Operations, Gateway, frontend y procesos Nx detenidos."
